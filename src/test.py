...
def test_model():
    ...
    test_dataset = PS2Dataset(data_dir='data/test', transform=get_transforms('val'), phase='val')
    test_loader = DataLoader(test_dataset, batch_size=16, shuffle=False, num_workers=8, pin_memory=True, prefetch_factor=2)
    ...
    evaluate_predictions(all_labels, all_preds_ensemble, 'Ensemble', 'roc_curve_test_ensemble.png')
    evaluate_predictions(all_labels, all_preds_stacking, 'Stacking', 'roc_curve_test_stacking.png')

def evaluate_predictions(all_labels, all_preds, method_name, save_path):
    test_auc = roc_auc_score(all_labels, all_preds)
    test_preds = (all_preds >= 0.5).astype(int)
    test_precision = precision_score(all_labels, test_preds)
    test_recall = recall_score(all_labels, test_preds)
    test_f1 = f1_score(all_labels, test_preds)
    test_accuracy = accuracy_score(all_labels, test_preds)

    print(f'{method_name} - AUC: {test_auc:.4f}, '
